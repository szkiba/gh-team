...
- [x] 2.1 Define `Resolver` interface returning `[]Repo` for a team slug
- [x] 2.1a Implement team-existence preflight (`GET /orgs/<org>/teams/<team-slug>`) shared by both strategies; cache result for the invocation
- [x] 2.2 Implement `permission` strategy
  - [x] 2.2.1 Fetch repos for the team with permission filter (`Admin` | `Maintain`)
...
- [x] 2.4 Apply `--include-archived` filter (default: drop archived)
